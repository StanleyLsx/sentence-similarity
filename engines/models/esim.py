...
class EsimModel(nn.Module, ABC):
    ...
    def forward(self, s1, s1_masks, s2, s2_masks):
        # embeds
        x1 = self.bn_embeds(self.embeds(s1).transpose(1, 2).contiguous()).transpose(1, 2)
        x2 = self.bn_embeds(self.embeds(s2).transpose(1, 2).contiguous()).transpose(1, 2)
